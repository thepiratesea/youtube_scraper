...
invidious_instances=("iv.nboeck.de" "vid.puffyan.us" "yt.artemislena.eu" "invidious.projectsegfau.lt" "invidious.no-logs.com" "anontube.lvkaszus.pl" "invidious.private.coffee")
while :
do
   randominvidious=$(($RANDOM % 7))
   invidious=${invidious_instances[$randominvidious-1]}
   checkinvidious=$(curl -s "https://$invidious/api/v1/videos/jNQXAC9IVRw" | jq -r '.title')
    ...
    if [[ $scrapeoption == "1" ]]; then ##Channel IDs only
        ...
        if [[ $videolistcount -gt "100" ]]; then
            ...
            do
                ...
                curl -s https://$invidious/api/v1/videos/$videoid?fields=recommendedVideos | jq -r '.recommendedVideos[].authorId' >> channel_ids.txt
                while :
                do
        ...
        if [[ $channellistcount -gt "100" ]]; then
            ...
            do
                ...
                if [[ $checkifexists == "" ]]; then
                    channelname=$(curl -s https://$invidious/api/v1/channels/$channelid | jq -r '.author')
                ...
                echo ""
                ####GET RECOMMENDED###
                curl -s https://$invidious/api/v1/videos/$videoid?fields=recommendedVideos | jq -r '.recommendedVideos[].videoId' >> videos.txt
                videocount=$(wc -l "videos.txt" | cut -d " " -f 1)
                channelcount=$(wc -l "channels.txt" | cut -d " " -f 1)
                metadatacount=$(wc -l "$scraperdir/metadata/youtube_video_dates.csv" | cut -d " " -f 1)
        ...
        while :
        do
            channelid=$(awk 'NR=='$channelnum'' "channels.txt")
            if [[ $channelid == "" ]]; then
                break
            fi
        ...
        checkiferror=$(curl -s https://$invidious/api/v1/channels/$channelid | jq -r '.error')
        ...
        playlisttitle=$(curl -s https://$invidious/api/v1/playlists/$playlistid | jq -r '.title')    
        playlistchannelname=$(curl -s https://$invidious/api/v1/playlists/$playlistid | jq -r '.author')    
        playlistchannelid=$(curl -s https://$invidious/api/v1/playlists/$playlistid | jq -r '.authorId')    
        playlistdescription=$(curl -s https://$invidious/api/v1/playlists/$playlistid | jq -r '.description')  
        ...
        do
            ...
            do
                ...
                checkerror=$(curl -s https://$invidious/api/v1/videos/$videoid | jq -r '.error')
            ...
            ###GET RECOMMENDED###
            curl -s https://$invidious/api/v1/videos/$videoid?fields=recommendedVideos | jq -r '.recommendedVideos[].videoId' >> videos.txt
            curl -s https://$invidious/api/v1/videos/$videoid?fields=recommendedVideos | jq -r '.recommendedVideos[].authorId' >> channels.txt
            ###DOWNLOAD VIDEO METADATA###
            source $scraperdir/youtube_scraper_download_video_metadata.sh
            ###GET VIDEO IDS###
...
if [[ $mainoption == "3" ]]; then
    ...
    read videoid
    echo "============================="
    channelid=$(curl -s https://$invidious/api/v1/videos/$videoid | jq -r '.authorId')    
    checkiferror=$(curl -s https://$invidious/api/v1/channels/$channelid | jq -r '.error')
    if [[ $checkiferror == "null" ]]; then
        ###DOWNLOAD VIDEO METADATA###
        source $scraperdir/youtube_scraper_download_video_metadata.sh
