...
checkiferror=$(curl -s https://$invidious/api/v1/videos/$videoid | jq -r '.error')
if [[ $checkiferror == "null" || $checkiferror == *"in your country"* ]]; then
    ...
    if [[ $checkifdateexists == "" ]] ; then
        if [[ $checkiferror == *"in your country"* ]]; then 
            ...
            videpochtime=$(curl -s https://$invidious/api/v1/videos/$videoid | jq -r '.published')
